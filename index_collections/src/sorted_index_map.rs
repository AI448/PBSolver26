...
/// 常にソートされたマップ
#[derive(Clone)]
pub struct SortedIndexMap<ValueT> {
...
impl<ValueT> Default for SortedIndexMap<ValueT> {
    #[inline(always)]
    fn default() -> Self {
        Self {
...
impl<ValueT> SortedIndexMap<ValueT> {
...
impl<ValueT> SortedIndexMap<ValueT> {
    ...
    pub fn get_mut(&mut self, index: usize) -> Option<&mut ValueT> {
        if let Some(&id) = self.position_array.get(index)
            && id != Self::NULL
        {
            return Some(&mut self.item_array[id].1);
        } else {
            return None;
        }
    }
    ...
    pub fn insert(&mut self, index: usize, value: ValueT) {
        ...
        if self.position_array[index] != Self::NULL {
            ...
            if self.item_array[current].0 <= new_item.0 {
                // 元の要素よりも大きくなった場合
                // current よりも大きい最小の要素を探索
                let next = self.search_next(current, Direction::RIGHT);
                if next == Self::NULL || new_item.0 <= self.item_array[next].0 {
                ...
                if next == Self::NULL || self.item_array[next].0 <= new_item.0 {
                    // 変更後の要素が next よりも小さいならソートし直す必要はない
                    self.item_array[current].1 = new_item.1;
                    return;
                }
    ...
    #[inline(always)]
    pub fn iter(&self) -> impl std::iter::DoubleEndedIterator<Item = (&usize, &ValueT)> {
        Iterator::new(self)
    }
    ...
    pub fn iter_mut(
        &mut self,
    ) -> impl std::iter::DoubleEndedIterator<Item = (&usize, &mut ValueT)> {
...
pub struct IteratorMut<'a, ValueT> {
    sorted_map: &'a mut SortedIndexMap<ValueT>,
    current: usize,
}

impl<'a, ValueT> IteratorMut<'a, ValueT> {
    const NULL: usize = <SortedIndexMap<ValueT>>::NULL;

    #[inline(always)]
    fn new(sorted_map: &'a mut SortedIndexMap<ValueT>) -> Self {
        Self {
...
impl<'a, ValueT> std::iter::Iterator for IteratorMut<'a, ValueT> {
    type Item = (&'a usize, &'a mut ValueT);
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        debug_assert!(
            self.current == Self::NULL
                || next == Self::NULL
                || self.sorted_map.item_array[self.current].0 <= self.sorted_map.item_array[next].0
        );
        self.current = next;
        if self.current == Self::NULL {
            return None;
        } else {
            let item = &mut self.sorted_map.item_array[self.current];
            unsafe {
                let index: &'a usize = std::mem::transmute(&item.0);
                let value: &'a mut ValueT = std::mem::transmute(&mut item.1);
                Some((index, value))
            }
        }
    }
...
impl<'a, ValueT> std::iter::DoubleEndedIterator for IteratorMut<'a, ValueT> {
    #[inline(always)]
    fn next_back(&mut self) -> Option<Self::Item> {
        ...
        debug_assert!(
            self.current == Self::NULL
                || next == Self::NULL
                || self.sorted_map.item_array[next].0 <= self.sorted_map.item_array[self.current].0
        );
        self.current = next;
        if self.current == Self::NULL {
            return None;
...
pub struct Iterator<'a, ValueT> {
    sorted_map: &'a SortedIndexMap<ValueT>,
    current: usize,
}

impl<'a, ValueT> Iterator<'a, ValueT> {
    const NULL: usize = <SortedIndexMap<ValueT>>::NULL;

    #[inline(always)]
    fn new(sorted_map: &'a SortedIndexMap<ValueT>) -> Self {
        Self {
            sorted_map: sorted_map,
            current: Self::NULL,
        }
...
impl<'a, ValueT> std::iter::Iterator for Iterator<'a, ValueT> {
...
impl<'a, ValueT> std::iter::DoubleEndedIterator for Iterator<'a, ValueT> {
    #[inline(always)]
    fn next_back(&mut self) -> Option<Self::Item> {
        let next = self.sorted_map.search_next(self.current, Direction::LEFT);
        debug_assert!(
...
impl<ValueT> std::fmt::Debug for SortedIndexMap<ValueT>
where
    ValueT: Debug,
{
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_map()
            .entries(self.iter().map(|(i, v)| (i, v)))
            .finish()
    }
}
